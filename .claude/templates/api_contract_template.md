...
> 💡 **填写说明**：
> - 将 `[模块名称]` 替换为实际模块名（如：用户管理、订单管理）
> - 将 `[资源]` 替换为实际资源名（如：users、orders），YAML 中需使用有效的路径格式
> - 将 `[项目简称]` 替换为项目简称（如：myapp、pawlog）
> - 版本：v1.0 开始，每次变更递增版本号
>
> 版本：v1.0
> 最后更新：YYYY-MM-DD
> 作者：@架构师（初始版本） / @后端工程师（详细设计与维护）
...
| 职责 | 负责人 | 说明 |
|------|--------|------|
| API 风格定义 | @架构师 | RESTful/gRPC/GraphQL 选择 |
| 核心接口设计 | @架构师 | 基于 PRD 和原型 |
| API 详细设计 | @后端工程师 | 字段、参数、错误码 |
| API 维护更新 | @后端工程师 | 变更时通知前端 |
| API 文档生成 | @后端工程师 | OpenAPI/Swagger |

**变更流程**：
1. 架构师在 QG-02 通过后创建 API 规范初始版本
2. 后端工程师在 QG-03 通过后补充完整 API 详情
3. 如有变更，后端工程师负责更新 API 规范并通知前端
