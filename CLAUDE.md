...
**Agent Team 与传统开发的区别**：
- **无固定周期**：Agent 24/7 工作，任务驱动而非时间驱动
- **异步交接**：通过文档索引和质量卡点进行交接
- **文档即契约**：所有决策和变更必须记录在文档中
- **用户确认**：关键节点由用户确认
...
# 标准开发流程（4 个核心质量卡点）
1. @项目经理 → 输出项目计划与任务拆解
   └─ 通过 QG-01 检查（项目规划 → 产品需求，**用户确认**）
2. @市场调研 → 输出市场调研报告（**可选**：如需要竞品分析或用户画像）
3. @产品经理 → 输出 PRD 文档 + **页面线框图**
   └─ 通过 QG-01 检查（产品需求 → 架构设计/原型，**用户确认**）
      ├──> @架构师开始设计
      └─> @UI/UX 设计师开始原型设计
4. @UI/UX 设计师 → 输出交互原型（**基于产品经理线框图**进行高保真设计）
5. @架构师 → 输出技术架构设计 + **环境配置规范**
   └─ 通过 QG-02 检查（架构设计 → 代码实现，**用户确认**）
      ├──> @后端工程师开始实现（提供 Mock 数据）
      └─> @前端工程师开始实现（使用 Mock 数据）
6. @AI 工程师 → 输出 AI 技术方案 (如需要，QG-01 通过后即可开始)
7. @后端工程师 → 实现 API 接口 + **提供 Mock 数据**（QG-02 通过后开始，读取架构师的配置文档）
8. @前端工程师 → 实现前端页面（QG-02 通过后开始，**使用 Mock 数据**）
9. 前后端自动进入联调阶段
   └─ 架构师创建联调报告 integration_report_[项目].md
   └─ 通过 QG-03 检查（联调完成 → 测试，**测试工程师确认**）
10. @测试工程师 → 输出测试报告（QG-03 通过后开始）
    └─ 通过 QG-04 检查（测试 → 部署，**用户确认**）
11. @DevOps → 部署上线（QG-04 通过后，读取架构师的配置文档）
```
...
- **联调规则**：前后端代码实现完成后，自动进入联调阶段，架构师创建联调报告
- **测试介入**：联调完成后，测试工程师开始测试
- **部署时机**：测试报告通过且用户确认后，DevOps 开始部署
