...
- **用户主导评审**：关键节点（PRD、原型、架构）由用户评审确认，用户可邀请其他 Agent 参加
...
7. **质量卡点**（新增）:
   ...
   - **关键卡点（QG-01, QG-02, QG-02.5, QG-03, QG-05）需要用户评审确认**
8. **变更管理**（新增）:
   - 需求变更需评估影响范围，决定是否需要重新通过质量卡点
   - P0 重大变更（影响产品定位）：重新通过完整质量卡点流程
   - P1 中等变更（影响部分功能）：重新通过受影响的质量卡点
   - P2 轻微变更（不影响已通过的卡点）：直接修改，更新文档版本
   - 变更流程详见 `.claude/templates/quality_gate.md#需求变更管理流程`
...
# 标准开发流程（包含质量卡点）
1. @项目经理 → 输出项目计划与任务拆解
   └─ 通过 QG-01 检查（项目规划 → 产品需求，**用户评审**）
2. @市场调研 → 输出市场调研报告（**可选**：如需要竞品分析或用户画像）
3. @产品经理 → 输出 PRD 文档 + **页面线框图**
   └─ 通过 QG-02 检查（产品需求 → 架构设计/原型，**用户评审**）
      ├──> @架构师开始设计（无需等待 QG-02.5）
      └─> @UI/UX 设计师开始原型设计
4. @UI/UX 设计师 → 输出交互原型（**基于产品经理线框图**进行高保真设计）
   └─ 通过 QG-02.5 检查（交互原型 → 技术确认，**用户评审**）
      ├──> 告知@架构师（如原型有重大变更则评估技术方案）
      └─> @前端工程师开始实现
5. @架构师 → 输出技术架构设计 + **环境配置规范**
   └─ 通过 QG-03 检查（架构设计 → 代码实现，**用户评审**）
      └─> @后端工程师开始实现
6. @AI 工程师 → 输出 AI 技术方案 (如需要)
7. @后端工程师 → 实现 API 接口（QG-03 通过后开始，读取架构师的配置文档）
   └─ 通过 QG-04 检查（代码实现 → 测试）
8. @前端工程师 → 实现前端页面（QG-02.5 通过后开始）
   └─ 通过 QG-04 检查（代码实现 → 测试）
9. @测试工程师 → 输出测试报告（QG-04 通过后开始）
   └─ 通过 QG-05 检查（测试 → 部署，**用户评审**）
10. @DevOps → 部署上线（QG-05 通过后，读取架构师的配置文档）
```
...
- **架构师在 QG-02 通过后即可开始，无需等待 QG-02.5**
...
**市场调研说明**：
- **可选环节**：内部工具、快速原型等项目可跳过市场调研
- **建议使用**：面向市场的产品、新赛道项目建议进行市场调研
- **产出物**：竞品分析、用户画像、市场趋势报告
